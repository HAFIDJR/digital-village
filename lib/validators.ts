...
export const reportCategorySchema = z.enum(
  ["INFRASTRUKTUR", "KEBERSIHAN", "KEAMANAN", "AIR_BERSIH", "LAYANAN_PUBLIK", "LAINNYA"],
...
export const registrySortSchema = z.enum(["name_asc", "name_desc", "newest", "dusun_asc"]);
...
/** Page number / size, reused by every paged read so the rules stay identical. */
 ...
 *
 * `URLSearchParams` renders `?status=A&status=B` as an array but `?status=A,B`
 * as one string, and the UI produces both forms, so both are accepted.
 */
...
/* -------------------------------------------------------------------------- */
/* Citizen reports workspace                                                   */
/* -------------------------------------------------------------------------- */

/**
 * Filters for "Laporan & Aspirasi Warga".
 *
 * Deliberately separate from `queueQuerySchema`: the two domains share only
 * pagination. Reports are filed by citizens against neighbourhoods and carry
 * categories; letters are filed against letter types and carry document
 * requirements. Merging them would force every filter to be optional for both.
 */
...
/** Parses `URLSearchParams` (or a plain record) into a ReportQuery. */
export function parseReportQuery(input: URLSearchParams | Record<string, unknown>): ReportQuery {
  const raw = input instanceof URLSearchParams ? Object.fromEntries(input) : input;
...
/** Parses `URLSearchParams` (or a plain record) into a RegistryQuery. */
