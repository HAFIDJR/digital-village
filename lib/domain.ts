...
/* -------------------------------------------------------------------------- */
/* Population registry vocabulary                                              */
/* -------------------------------------------------------------------------- */
...
export const GENDER_SHORT: Record<string, string> = { L: "L", P: "P" };
...
export const MARITAL_STATUS_SHORT: Record<string, string> = {
  BELUM_MENIKAH: "Belum Menikah",
  KAWIN: "Kawin",
  CERAI_HIDUP: "Cerai Hidup",
  CERAI_MATI: "Cerai Mati",
 ...
 * Registry status.
 *
 * `TIDAK_DIKENAL` is not "pending" — a record whose subject cannot be traced is
 * a data-quality problem the operator has to fix, so it carries the rose tone
 * that the rest of the system reserves for work that is stuck.
 */
export const RESIDENT_STATUS: Record<string, { label: string; tone: Tone; hint: string }> = {
  AKTIF: {
    label: "Aktif",
    tone: "approved",
    hint: "Terdaftar sebagai penduduk aktif desa.",
  },
  PINDAH_KELUAR: {
    label: "Pindah Keluar",
    tone: "progress",
    hint: "Sudah mengajukan surat pindah keluar dan tidak dihitung dalam DPT desa.",
  },
  MENINGGAL: {
    label: "Meninggal",
    tone: "neutral",
    hint: "Tercatat meninggal; akta kematian sudah diterima desa.",
  },
  TIDAK_DIKENAL: {
    label: "Perlu Verifikasi",
    tone: "rejected",
    hint: "Data alamat atau NIK tidak dapat diverifikasi petugas.",
  },
};

export const RESIDENT_STATUS_ORDER = [
  "AKTIF",
  "PINDAH_KELUAR",
  "MENINGGAL",
  "TIDAK_DIKENAL",
...
export function welfareTone(value: string | null | undefined): Tone {
  ...
  return WELFARE_CLASS_TONE[value] ?? "neutral";
}
...
export const SIGNATURE_STATUS: Record<string, { label: string; tone: Tone }> = {
  MENUNGGU: { label: "Menunggu TTD", tone: "pending" },
  DITANDATANGANI: { label: "Ditandatangani", tone: "approved" },
  KEDALUWARSA: { label: "Kedaluwarsa", tone: "rejected" },
  DIBATALKAN: { label: "Dibatalkan", tone: "neutral" },
};

/* -------------------------------------------------------------------------- */
/* Sort labels shared by the list toolbars                                     */
/* -------------------------------------------------------------------------- */

export const REPORT_SORT_LABEL: Record<string, string> = {
  newest: "Terbaru masuk",
  oldest: "Terlama masuk",
  priority_desc: "Prioritas tertinggi",
  unanswered_first: "Belum ditanggapi lebih dulu",
};

export const REGISTRY_SORT_LABEL: Record<string, string> = {
  name_asc: "Nama A–Z",
  name_desc: "Nama Z–A",
  newest: "Terbaru terdata",
  dusun_asc: "Wilayah dusun",
};

export const ARCHIVE_SORT_LABEL: Record<string, string> = {
  submitted_desc: "Terbaru diselesaikan",
  submitted_asc: "Terlama diselesaikan",
  sla_asc: "Batas waktu",
  priority_desc: "Prioritas tertinggi",
};
