...
async function populate(db: Db, options: SeedOptions, rng: Next, started: number) {
  ...
  const buildRequest = (opts: {
    ...
    return {
      ...
      dueAt,
      verifiedAt:
        opts.status === "PENDING_VERIFIKASI" || opts.status === "BERKAS_TIDAK_LENGKAP"
          ? null
          : new Date(opts.submittedAt.getTime() + intBetween(rng, 20, 300) * 60_000),
      signedAt:
        opts.status === "DITANDATANGANI" || opts.status === "SIAP_DIAMBIL" || opts.status === "SELESAI"
          ? new Date(opts.submittedAt.getTime() + intBetween(rng, 240, 1400) * 60_000)
          : null,
      completedAt:
        opts.status === "SELESAI"
          ? new Date(opts.submittedAt.getTime() + intBetween(rng, 400, 2000) * 60_000)
          : null,
